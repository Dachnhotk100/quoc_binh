...
diem_trung_binh <- mean(exam_scores)

abline(v = diem_trung_binh, col = "red", lwd = 2, lty = 2)
...
boxplot(Petal.Length ~ Species,
        ...
        border = c("steelblue", "darkgreen", "tomato"),
        main   = "So sánh Petal.Length giữa 3 loài Iris",
        xlab   = "Loài",
...
legend("topleft",
       ...
       fill   = c("lightblue", "lightgreen", "lightsalmon"),
       title  = "Loài hoa",
       cex    = 0.85)
# - Loài có petal dài nhất: Virginica (trung vị ~5.5 cm)
# - Loài có độ biến thiên lớn nhất: Versicolor và Virginica (IQR rộng hơn)
# - Outliers: Versicolor có một vài điểm ngoại lai ở phía dưới
...
mau_cyl <- c("4" = "steelblue", "6" = "darkorange", "8" = "tomato")

plot(mpg ~ hp,
     data = mtcars,
     col  = mau_cyl[as.character(mtcars$cyl)],
     pch  = 19,
     cex  = 1.3,
...
legend("topright",
       legend = c("4 xi-lanh", "6 xi-lanh", "8 xi-lanh"),
       col    = c("steelblue", "darkorange", "tomato"),
       pch    = 19,
       title  = "Số xi-lanh (cyl)",
       cex    = 0.85)
# 5. Nhận xét về mối quan hệ giữa hp và mpg
# - Có mối quan hệ âm rõ ràng: hp tăng thì mpg giảm
# - Xe 4 xi-lanh có hp thấp và mpg cao; xe 8 xi-lanh ngược lại
# - Đường hồi quy xác nhận xu hướng giảm dần
...
par(mfrow = c(2, 2),          # Layout 2 hàng x 2 cột
    mar   = c(4, 4, 3, 1))    # Lề: dưới, trái, trên, phải

# --- Biểu đồ 1: Histogram của hp ---
hist(mtcars$hp,
     col    = "steelblue",
     border = "white",
     main   = "Histogram của Horsepower",
     xlab   = "Horsepower (hp)",
     ylab   = "Tần số",
     breaks = 10)

# --- Biểu đồ 2: Box plot của hp ---
boxplot(mtcars$hp,
        col    = "lightsalmon",
        border = "tomato",
        main   = "Box Plot của Horsepower",
        ylab   = "Horsepower (hp)",
        horizontal = FALSE)

# --- Biểu đồ 3: Box plot so sánh hp theo cyl ---
boxplot(hp ~ cyl,
        data   = mtcars,
        col    = c("lightblue", "lightgreen", "lightsalmon"),
        border = c("steelblue", "darkgreen", "tomato"),
        main   = "HP theo số xi-lanh",
        xlab   = "Số xi-lanh (cyl)",
        ylab   = "Horsepower (hp)")

# --- Biểu đồ 4: Scatter plot hp vs mpg ---
plot(mpg ~ hp,
     data = mtcars,
     col  = mau_cyl[as.character(mtcars$cyl)],
     pch  = 19,
     ...
     xlab = "Horsepower (hp)",
     ylab = "Miles per Gallon (mpg)")
abline(lm(mpg ~ hp, data = mtcars), col = "black", lwd = 2, lty = 2)
...
# Ma trận doanh thu: hàng = sản phẩm, cột = quý
sales_matrix <- matrix(c(Q1, Q2, Q3, Q4),
                       nrow     = 4,
                       ncol     = 4,
                       dimnames = list(products, c("Q1", "Q2", "Q3", "Q4")))

# Tổng doanh thu mỗi quý và mỗi sản phẩm
tong_quy        <- colSums(sales_matrix)
tong_san_pham   <- rowSums(sales_matrix)

# Bảng màu
mau_sp <- c("steelblue", "darkorange", "mediumseagreen", "tomato")
mau_qy <- c("lightblue", "lightyellow", "lightgreen", "lightsalmon")
...
barplot(t(sales_matrix),          # Chuyển vị để nhóm theo sản phẩm
        beside  = TRUE,
        col     = mau_qy,
        border  = NA,
        main    = "Doanh thu theo Sản phẩm và Quý",
        xlab    = "Sản phẩm",
        ylab    = "Doanh thu",
        legend.text = c("Q1", "Q2", "Q3", "Q4"),
        args.legend = list(x = "topleft", cex = 0.75, bty = "n"),
        names.arg   = c("SP A", "SP B", "SP C", "SP D"))
# 2. Vẽ line plot cho từng sản phẩm qua 4 quý
quarters <- 1:4
plot(quarters, sales_matrix[1, ],
     type = "b", pch = 19, col = mau_sp[1],
     ylim = c(90, 230), lwd = 2,
     main = "Xu hướng doanh thu theo quý",
     xlab = "Quý", ylab = "Doanh thu",
     xaxt = "n")
axis(1, at = 1:4, labels = c("Q1", "Q2", "Q3", "Q4"))

for (i in 2:4) {
  lines(quarters, sales_matrix[i, ], type = "b", pch = 19,
        col = mau_sp[i], lwd = 2)
}
...
barplot(tong_san_pham,
        ...
        ylab   = "Tổng doanh thu",
        names.arg = c("SP A", "SP B", "SP C", "SP D"),
        ylim   = c(0, max(tong_san_pham) * 1.15))

# Thêm nhãn giá trị trên mỗi cột
text(x      = barplot(tong_san_pham, plot = FALSE),
     y      = tong_san_pham + 10,
     labels = tong_san_pham,
     cex    = 0.85,
     font   = 2)
